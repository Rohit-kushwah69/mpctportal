...
class FrontController {
    ...
    static sendVerifymail = async (name, email, user_id) => {
        ...
        let info = await transporter.sendMail({
            ...
            html: "<p>Hii " +
                name +
                ',Please click here to <a href="http://localhost:2000/register/verify?id=' +
                user_id +
    ...
    static verifyMail = async (req, res) => {
        try {
            ...
            if (updateinfo) {
                let token = jwt.sign({ ID: updateinfo.id }, 'hksafh534fet35ge22')
        ...
        try {
            ...
            if (!user) {
                ...
                if (isMatch) {
                    ...
                    if (user.role == 'admin') {
                        // token
                        const token = jwt.sign({ ID: user.id }, 'hksafh534fet35ge22')
                        // console.log(token)
                        res.cookie('token', token)
    ...
    static sendEmail = async (name, email, token) => {
        ...
        let info = await transporter.sendMail({
            ...
            html: "<p>Hii " +
                name +
                ',Please click here to <a href="http://localhost:2000/reset-password?token=' +
                token +
